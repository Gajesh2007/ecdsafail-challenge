//! `kaliski::iteration1` — verbatim split of the original `kaliski` module.
...
pub(crate) fn kaliski_iteration_bulk_prefix3_backward(
    ...
    // Reverse STEP 9.
    if let Some((cr, cs)) = coeff {
        b.set_phase("bk_bulk_coeff_step9_cswap");
        coeff_channel_cswap(b, a_f, cr, cs);
    }
    ...
    if let Some((cr, _cs)) = coeff {
        b.set_phase("bk_bulk_coeff_step8_halve");
    ...
    // Reverse STEP 4.
    if let Some((cr, cs)) = coeff {
        b.set_phase("bk_bulk_coeff_step4_sub");
    ...
    b.set_phase("bk_bulk_step4");
    {
        ...
        let add_width = if bulk_backward_step4_truncated_vadd_enabled()
            && iter_idx >= bulk_backward_step4_truncated_vadd_min_iter()
        {
            2 * n - iter_idx
        } else {
            n
        ...
        if std::env::var("KAL_STEP4_CTRL_LOWQ").ok().as_deref() == Some("1") {
            ...
            // The inverse v_w add below is truncated for late bulk-prefix steps.
            // Bits above add_width are only needed to undo the prior s -= r stage;
            // after that subtraction they are still add_f&r and can be HMR-cleaned
            // directly.  Transforming them through u is dead work.
            let transform_width = add_width;
            for i in 0..transform_width {
                b.cx(r[i], u[i]);
            }
            for i in 0..transform_width {
            ...
            // For late bulk-prefix iterations, forward STEP 4 only subtracted the
            // low `2n-k` denominator bits from v_w. Reverse still needs the full
            // transform above, because forward added the full r-lane into s, but
            // after that transform tmp[i>=2n-k] is known zero.  The inverse v_w
            // add can therefore be truncated without changing the map.
    ...
    // Reverse STEP 3.
    if let Some((cr, cs)) = coeff {
        b.set_phase("bk_bulk_coeff_step3_cswap");
        coeff_channel_cswap(b, a_f, cr, cs);
    }
