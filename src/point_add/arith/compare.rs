//! Reversible comparison predicates.
//!
//! Computes less-than / greater-than / equal-to-zero results into a flag qubit
//! (`cmp_*`), exposes scoped `with_lt` / `with_gt` / `with_eq_zero` wrappers,
//! and provides the polarized multi-control helpers (`mcx2_polar`,
//! `mcx3_polar`, `or_step`).
...
pub(crate) fn cmp_gt_const_n1(b: &mut B, r: &[QubitId], c: U256, flag: QubitId) {
    let n1 = r.len();
...
pub(crate) fn toggle_eq_zero_flag_fast(b: &mut B, v: &[QubitId], flag: QubitId) {
    let n = v.len();
    assert!(n > 0);
    if n == 1 {
        b.x(v[0]);
    ...
    let or_chain = b.alloc_qubits(n - 1);
    or_step(b, v[0], v[1], or_chain[0]);
    for i in 1..n - 1 {
        or_step(b, or_chain[i - 1], v[i + 1], or_chain[i]);
    }
...
pub(crate) fn cmp_lt_into_borrowed_cin(b: &mut B, u: &[QubitId], v: &[QubitId], flag: QubitId, c_in: QubitId) {
    let n = u.len();
    assert_eq!(n, v.len());
...
pub(crate) fn cmp_lt_into_fast_borrowed_carries(
    b: &mut B,
    u: &[QubitId],
    v: &[QubitId],
    flag: QubitId,
