//! `multiply::karatsuba1` — verbatim split of the original `multiply` module.
...
pub(crate) fn mod_mul_write_into_zero_acc_karatsuba_lowq_with_tmp_ext(
    b: &mut B,
    acc: &[QubitId],
    x: &[QubitId],
    y: &[QubitId],
    ...
    let h = n / 2;
    let z1_reg = b.alloc_qubits(2 * (h + 1));
    b.set_phase("kara_lowq_fwd");
    karatsuba_forward_lowq(b, x, y, tmp_ext, &z1_reg);
    b.set_phase("kara_lowq_solinas");

    let lo: Vec<QubitId> = tmp_ext[0..n].to_vec();
    let hi: Vec<QubitId> = tmp_ext[n..2 * n].to_vec();
    let lowq_shift22 = std::env::var("KARA_WRITE_LOWQ_SHIFT22").ok().as_deref() == Some("1");
    b.set_phase("sol_lowq_addlo");
    if lowq_shift22 {
        mod_add_qq(b, acc, &lo, p);
    } else {
        mod_add_qq_fast_from_zero(b, acc, &lo, p);
    }
    b.set_phase("sol_lowq_add0");
    if lowq_shift22 {
        mod_add_qq(b, acc, &hi, p);
    } else {
        mod_add_qq_fast(b, acc, &hi, p);
    ...
    b.set_phase("sol_lowq_add10");
    if lowq_shift22 {
        mod_add_qq(b, acc, &hi, p);
    } else {
        mod_add_qq_fast(b, acc, &hi, p);
    }
    b.set_phase("kara_lowq_solinas_shift22L");
    let (spill, flag_inv, ovf) = if lowq_shift22 {
        mod_shift_left_by_k_lowq(b, &hi, p, 22)
    } else {
        mod_shift_left_by_k(b, &hi, p, 22)
    };
    b.set_phase("kara_lowq_solinas_post32_add");
    mod_add_qq(b, acc, &hi, p);
    b.set_phase("kara_lowq_solinas_shift22R");
    if lowq_shift22 {
        mod_shift_right_by_k_lowq(b, &hi, p, 22, spill, flag_inv, ovf);
    } else {
        mod_shift_right_by_k(b, &hi, p, 22, spill, flag_inv, ovf);
    }
    b.set_phase("kara_lowq_solinas_post_halve");
    for _ in 0..10 {
        mod_halve_inplace_fast(b, &hi, p);
    }

    b.set_phase("kara_lowq_inv");
    karatsuba_inverse_lowq(b, x, y, tmp_ext, &z1_reg);
    b.free_vec(&z1_reg);
}

pub(crate) fn mod_mul_write_into_zero_acc_karatsuba_lowq(
    b: &mut B,
    acc: &[QubitId],
    x: &[QubitId],
    y: &[QubitId],
    ...
    let tmp_ext = b.alloc_qubits(2 * acc.len());
    mod_mul_write_into_zero_acc_karatsuba_lowq_with_tmp_ext(b, acc, x, y, p, &tmp_ext);
    b.free_vec(&tmp_ext);
}
...
pub(crate) fn mod_mul_add_into_acc_karatsuba2(
    b: &mut B,
    acc: &[QubitId],
    x: &[QubitId],
    y: &[QubitId],
